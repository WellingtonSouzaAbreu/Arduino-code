...
void setup(void) {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    htmlPage += "        axios.get(\"http://192.168.2.19:81/esp32/\" + stateOfSwitch)";
    htmlPage += "             .then(res => setStateOfSwitchLabel(stateOfSwitch))";
  ...
  server.on("/esp32/on", HTTP_GET, []() {
    HTTPClient http;

    http.begin("192.168.2.251", 80, "/on");
    ...
    if (httpCodeGet) {
      ...
      if (httpCodeGet == 200) {
        ...
        Serial.print("Ocorreu algum erro!");
      }
